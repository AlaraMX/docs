...
<AccordionGroup>
  ...
  <Accordion title="date_field — en la fecha de un campo del pase">
    ...
    <ParamField body="offset_days" type="integer" default="0">
      Días de anticipación respecto a la fecha del campo. Cada día la regla busca los pases cuya fecha cae dentro de `offset_days` días, así que `3` la dispara **tres días antes** y `-1` un día después. Debe ser un número entero.
    </ParamField>

    <ParamField body="ignore_year" type="boolean" default="false">
      Con `true` compara solo día y mes, ignorando el año guardado. Es lo habitual para cumpleaños y aniversarios.
    </ParamField>
    ...
    ```json
    {
      "field_name": "vencimiento_membresia",
      "recurrence": "once",
      "offset_days": 3
    }
    ```
...
Devuelve los pases que **cumplen la condición en este momento**, sin ejecutar ninguna acción. Sirve para comprobar el alcance de una regla antes de activarla: si te devuelve miles de pases o ninguno, seguramente la condición no es la que querías.
...
<ResponseField name="pass_ids" type="array">
  Identificadores de los pases que cumplen la condición ahora mismo. Viene vacío si ninguno la cumple.
</ResponseField>

Funciona con la regla **tal como está guardada**, así que primero guarda los cambios y luego previsualiza. La previsualización no consume el límite de una ejecución por pase: puedes llamarla las veces que quieras.

<Note>
  La previsualización evalúa la condición contra el estado actual, no contra el futuro. En `date_field` y `datetime_field` te dice qué pases se dispararían **hoy**, no los que se dispararán más adelante.
</Note>

<Warning>
  Las reglas de tipo `stamp_threshold` no admiten previsualización y devuelven `400 RULE_PREVIEW_UNSUPPORTED`. Se disparan por un evento —el sellado— en el momento en que ocurre, no sobre un conjunto de pases que se pueda consultar de antemano.
</Warning>
...
```json Respuesta 400
{
  "error": "preview trigger rule failed",
  "code": "RULE_PREVIEW_UNSUPPORTED",
  "message": "Preview is not available for event-driven trigger rules"
}
```
</CodeGroup>
